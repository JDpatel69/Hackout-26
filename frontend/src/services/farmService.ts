...
export const farmService = {
  ...
  async viewCarbonCredits(_operatorId: string): Promise<CarbonCredit[]> {
    // Backend now serves this at /farms/credits/me (scoped to the JWT user)
    // to avoid a FastAPI path-param collision with /farms/{farm_id}.
    return api<CarbonCredit[]>('/farms/credits/me');
  },
};
